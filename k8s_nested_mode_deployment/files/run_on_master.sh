...
function deploy_on_ubuntu ()
{
   ...
   if [ !$RETVAL ]; then
      echo "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"
      echo "Successfully deployed k8s on master"
      echo "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"
   else
      echo "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"
      echo "FAILED to deploy k8s on master !!!!!"
      echo "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"
      exit 1
   fi

   echo "KUBELET_KUBEADM_ARGS=--cgroup-driver=cgroupfs" > /var/lib/kubelet/kubeadm-flags.env
